...
class LouvreMuseum:
    ...
    def remove_special_event(self, special_event: SpecialEvent):
        if special_event in self.__special_events:
            ...
            return True
        return False
def test_case_purchase_tickets():
    print("Test Case C: Purchasing Tickets for an Event")
    ticket_pricing = TicketPricing()
    price_adult = ticket_pricing.calculate_discounted_price("adult", False)
    special_event_ticket = SpecialEvent("SE123", date.today(), "V12345", price_adult, "E456", "Special Exhibition Opening", date(2024, 4, 15), Location.EXHIBITION_HALLS)

    assert special_event_ticket.get_price() == price_adult, "Ticket price calculation failed."
    print("Ticket purchased successfully for the event.")
    print(f"Event Name: {special_event_ticket.get_event_name()}, Price: {special_event_ticket.get_price()}")
test_case_purchase_tickets()