...
class LouvreMuseum:
    ...
    def remove_special_event(self, special_event: SpecialEvent):
        if special_event in self.__special_events:
            ...
            return True
        return False
def test_case_new_exhibition():
    print("Test Case B: Opening a New Exhibition")
    louvre = LouvreMuseum("Louvre Museum", "Abu Dhabi, UAE", "9:00 AM - 6:00 PM")
    new_exhibition = Exhibition(1, "Renaissance Art", date(2024, 1, 1), date(2024, 12, 31), "Gallery 1")
    louvre.add_exhibition(new_exhibition)

    assert new_exhibition in louvre.get_exhibitions(), "New exhibition was not added successfully."
    print("New exhibition opened successfully in the museum.")
    for exhibition in louvre.get_exhibitions():
        print(f"Exhibition ID: {exhibition.get_exhibition_id()}, Name: {exhibition.get_name()}")
test_case_new_exhibition()
