...
def test_case_add_new_art():
    print("Test Case A: Adding New Art to the Museum")
    louvre = LouvreMuseum("Louvre Museum", "Abu Dhabi, UAE", "9:00 AM - 6:00 PM")
    new_artifact = Artifact(1, "Mona Lisa", "A portrait of a woman by Leonardo da Vinci", "Over 500 years", "Italian", True)
    louvre.add_artifact(new_artifact)

    assert new_artifact in louvre.get_artifacts(), "New art was not added successfully."
    print("New art added successfully to the museum.")
    for artifact in louvre.get_artifacts():
        print(f"Artifact ID: {artifact.get_artifact_id()}, Name: {artifact.get_name()}")

# Execute the test case
test_case_add_new_art()
