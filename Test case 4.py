...
class LouvreMuseum:
    ...
    def remove_special_event(self, special_event: SpecialEvent):
        if special_event in self.__special_events:
            ...
            return True
        return False
def test_case_payment_receipt():
    print("Test Case D: Displaying Payment Receipt for Ticket Purchasing")
    ticket_pricing = TicketPricing()
    price_group = ticket_pricing.calculate_discounted_price("adult", True)  # Assuming a group purchase gets a discount
    special_event_ticket_group = SpecialEvent("SE124", date.today(), "V54321", price_group, "E789", "Art History Seminar", date(2024, 5, 20), Location.OUTDOOR_SPACES)

    print("Payment Receipt for Group Ticket Purchase:")
    print(f"Event Name: {special_event_ticket_group.get_event_name()}, Total Price: {special_event_ticket_group.get_price()}")
    assert special_event_ticket_group.get_price() < ticket_pricing.get_price_adult(), "Group discount was not applied."
    print("Group discount applied successfully.")
test_case_payment_receipt()